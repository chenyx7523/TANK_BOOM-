...
namespace Complete
{
    ...
    public class TankManager
    {
        ...
        [HideInInspector] public int m_Wins;                    // 这个玩家到目前为止的胜利次数。


        private TankMovement m_Movement;                        // 参考坦克的移动脚本，用于禁用和启用控制。
        private TankFire m_Shooting;                        // 参考坦克的射击脚本，用于禁用和启用控制。
        ...
        public void Setup()
        {
            ...
            m_Shooting = m_Instance.GetComponent<TankFire>();
            m_CanvasGameObject = m_Instance.GetComponentInChildren<Canvas>().gameObject;

            // 设置玩家号码，使其在脚本中保持一致。
            m_Movement.m_Playernum = m_PlayerNumber;
            m_Shooting.m_Playernum = m_PlayerNumber;
