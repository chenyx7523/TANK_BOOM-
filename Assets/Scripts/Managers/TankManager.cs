...
using UnityEngine;
        ...
        * 决定玩家在何时可以控制自己的坦克
        */
[SerializeField]        //强制可序列化（使其在ui界面中显示）
public class TankManager 
{

    public Color m_PlayerColor;               // 这是这个坦克将要生成的颜色。
    public Transform m_Bron;             // 坦克出生的位置和方向。
    public int m_Playernum;                 // 用来指定玩家。
    public string m_ColorPlayerName;        // 一串代表玩家的数字，颜色与他们的坦克相匹配。
    public int m_WinTime;                   // 这个玩家到目前为止的胜利次数。
    public GameObject m_Instance;           // 创建对象时对其实例的引用。
    ...
    public void Setup()
    {
        ...
        // 设置玩家号码，使其在脚本中保持一致。
        m_Movement.m_Playernum = m_Playernum;   
        m_Fire.m_Playernum = m_Playernum;

        // 根据坦克的颜色和玩家的号码创建一个字符串，使用正确的颜色表示“玩家胜利”的显示字符。
        m_ColorPlayerName = "<color=#" + ColorUtility.ToHtmlStringRGB(m_PlayerColor) + "PLAYER" + m_Playernum + "</color>";
